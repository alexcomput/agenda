...
enum OrdemOptions { oderaz, orderza }
...
class _HomepageState extends State<Homepage> {
          ...
          PopupMenuButton<OrdemOptions>(
            itemBuilder: (context) => <PopupMenuEntry<OrdemOptions>>[
              const PopupMenuItem<OrdemOptions>(
                ...
                value: OrdemOptions.oderaz,
              ),
              const PopupMenuItem<OrdemOptions>(
                child: Text("Ordenar de Z-A"),
                value: OrdemOptions.orderza,
              )
            ],
            onSelected: _ondenList,
          )
        ],
      ),
              ...
              children: <Widget>[
                ...
                Padding(
                  ...
                  child: Column(
                    ...
                    children: <Widget>[
                      Text(
                        contacts[index].name.length > 20
                            ? '${contacts[index].name.substring(0, 20)}...'
                        : contacts[index].name ?? "",
                        style: TextStyle(
                      ...
                      Text(
                        contacts[index].email.length > 20
                            ? '${contacts[index].email.substring(0, 20)}...'
  ...
  void _getAllContacts() {
    contactHelp.getAllContact().then((list) {
      setState(() {
        contacts = list;
      });
    });
  ...
  void _ondenList(OrdemOptions result) {
    switch (result) {
      case OrdemOptions.oderaz:
        contacts.sort((a, b) {
          return a.name.toLowerCase().compareTo(b.name.toLowerCase());
        });
        break;
      case OrdemOptions.orderza:
        contacts.sort((a, b) {
